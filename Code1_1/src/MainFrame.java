	...
	private final ActionListener listener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				{
					new Server(port, MainFrame.this);
				}else
				{
					Sender client;
					if (source == sawButton) {
						client = new SAW(loss, MainFrame.this);
					} else {
						client = new GBN(loss, Config.GBN_WINDOW_SIZE, MainFrame.this);
					}
					
					try (InputStream inputStream = new FileInputStream("COSC635_P2_DataSent.txt")) {
						client.send(host, port, inputStream);
					} catch (Exception e2) {
						throw new RuntimeException(e2);
					}
				}
