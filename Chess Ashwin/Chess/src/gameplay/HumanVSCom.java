		...
		synchronized(cBoard){
			...
			if(temp != null){
				...
				if(temp.color == myColor){
					...
					// neu la quan minh thi dub vao from
					if(isRoque == false){
				...
				synchronized (cBoard) {
					synchronized(isEnemySelect){
						if(isEnemySelect && !isGameOver){
							synchronized(com){
								// tuy thuoc vao so quan co tren ban co ma dat do sau
								/*if((cBoard.cArray[0].n + cBoard.cArray[1].n) <= 4)
									com.setDepth(depth + 2);
								else if((cBoard.cArray[0].n + cBoard.cArray[1].n) <= 15)
									com.setDepth(depth + 1);
								else
									com.setDepth(depth);*/
