	...
	private final ActionListener listener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				{
					new FileServer().start(port, MainFrame.this);
				}else
				{
					try (InputStream inputStream = new FileInputStream("COSC635_P2_DataSent.txt")) {
					    if (source == sawButton) {
					        new SAWTransferer().send(host, port, loss, inputStream, MainFrame.this);
					    } else {
					        new GBNTransferer().send(host, port, loss, Config.GBN_WINDOW_SIZE, inputStream, MainFrame.this);
					    }
					} catch (Exception e2) {
						throw new RuntimeException(e2);
					}
